...
Str8 str8_valist(Arena* arena, const char* fmt, va_list valist)
{
  ...
  int size_written   = vsprintf_s((char*)buffer.data, buffer.count, fmt, valist);
...
// TODO: This need a better names, since this uses a view and doesnt realloc the string 
...
// TODO: Add a separate call that would do the same but have this allocated on an array onto the arena.
//       The string might not be compied, thats fine if they dont, then we might just have the Str8 and the U64
//       be opt params to the param list so we dont have to make a new type
Str8_list str8_split_ex(Arena* arena, Str8 str, Str8 spliter, Str8_match_flags flags, B32 include_empty_once_after_split)
{
  ...
  if (spliter.count <= str.count)
  {
    ...
    if (spliter.count != 0)
    {
      for (U64 i = 0; (i < str.count) && (i + spliter.count <= str.count);)
      {
        ...
        if (str8_match(test_substring, spliter, flags))
        {
          ...
          if (new_substring.count > 0 || include_empty_once_after_split)
          {
    ...
    // At this point we either have had a splitter in the past and now have just a string to add 
    // to the list OR we just had a splitter and in that case we will have the 
    // word_start_index == str and we might stil want to add an empty string in
    if (word_start_index < str.count || include_empty_once_after_split)
    {
...
{
  return str8_split_ex(arena, str, spliter, flags, false);
}

void str8_split_into_array(Arena* arena, Str8 str, Str8 spliter, Str8_match_flags flags, B32 include_empty_once_after_split, Str8** out_parts, U64* out_parts_count)
{
  Scratch scratch = get_scratch(&arena, 1);
  
  Str8_list str_parts = str8_split_ex(scratch.arena, str, spliter, flags, include_empty_once_after_split);
  Str8* str_arr = ArenaPushArr(arena, Str8, str_parts.node_count);
  U64 node_index = 0;
  for (Str8_node* node = str_parts.first; node; node = node->next, node_index += 1)
  {
    str_arr[node_index] = node->str;
  }

  *out_parts       = str_arr;
  *out_parts_count = str_parts.node_count;
