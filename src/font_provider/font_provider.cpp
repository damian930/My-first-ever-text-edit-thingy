...
B32 __fp_try_to_generate_grey_scale_font_atlas_image(
  ...
  stb_pack_range.num_chars                        = (int)rangeU64_count(unicode_range_to_load); // todo: This truncated u64 to int
...
FP_Font fp_load_font(Str8 ttf_file_path, F32 font_size, RangeU64 unicode_range_to_load)
{
  ...
  FP_Font* result_font = {};
  {
    ...
    result_font->codepoints_data_arr = ArenaPushArr(fp_state->state_arena, FP_Codepoint_data, rangeU64_count(unicode_range_to_load));;
  ...
  stbtt_packedchar* packed_char_data_arr = ArenaPushArr(scratch.arena, stbtt_packedchar, rangeU64_count(unicode_range_to_load));
  ...
  // Setting font data
  {
    ...
    for (U64 codepoint_index = 0; codepoint_index < rangeU64_count(unicode_range_to_load); codepoint_index += 1)
    {
...
{
  if (!rangeU64_within(font.codepoint_range, unicode_codepoint)) { return FP_Codepoint_data{}; }
